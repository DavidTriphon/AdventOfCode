...
public class Test12
{
   private static final String FILE_LOC_PREFIX =
      ReaderUtil.TEST_RESOURCES_LOCATION + "years/y2019/12/";
   ...
   public void printB() throws IOException
   {
      ...
      String fileString = ReaderUtil.getFileString(filename);
   ...
   public void testTimeB()
   {
      try
      {
         NBody nBody = NBody.fromString(ReaderUtil.getFileString(getFileName("b", START)));
         
         // the clone is expected to be the nbody
         assertEquals(nBody, new NBody(nBody), "A clone should be equal");
         
         long expLoopTime =
            Long.parseLong(ReaderUtil.getFileString(getFileName("b", TIME)));
   ...
   private static void testFunc(String filename)
   {
      try
      {
         ...
         NBody nBody = NBody.fromString(ReaderUtil.getFileString(startFilename));
         Expected exp = getExpected(ReaderUtil.getFileString(expectedFilename));
